...
class Edd_Terms {
	...
	static function extra_download_category_fields( $tag ) {    //check for existing featured ID
		$t_id     = $tag->term_id;
        ...
        <tr class="form-field">
            ...
            <td>
               <?php echo self::image_download_uploader_field('featured_image', $cat_meta); ?>
            </td>
        </tr>
        <script>
	...
	static function save_extra_download_category_fields($term_id) {
		if ( isset( $_POST['featured_image'] ) ) {
			update_term_meta(sanitize_key($_POST['tag_ID']), '_featured_image', sanitize_text_field($_POST['featured_image']));
		}
    }

    static function image_download_uploader_field( $name, $value = '') {
