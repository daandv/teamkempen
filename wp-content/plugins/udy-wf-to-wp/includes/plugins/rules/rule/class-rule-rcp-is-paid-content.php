...
class Rule_Rcp_Is_Paid_Content extends Rule {
	...
	public function __construct( $condition = '', $values = array() ) {
		...
		$this->key   = 'rcp_is_paid_content';
		$this->label = __('[RCP] Paid Content', i18n::$textdomain);
	}
	...
	public function get_valid_conditions() {
		return array(
			'in'    => __('Is paid content', i18n::$textdomain),
			'notin' => __('Is not paid content', i18n::$textdomain),
		);
	}
	...
	public function is_matched_by( $post_id ) {
		...
		switch ( $condition ) {
			case 'in':
				return rcp_is_paid_content( $post_id );
				break;
			case 'notin':
				return ! rcp_is_paid_content( $post_id );
				break;
