...
class Rule_Woo_Has_Subcategories extends Rule {
	...
	public function __construct( $condition = '', $values = array() ) {
		...
		$this->key   = 'woo_has_subcategories';
		$this->label = __('[WOO] Has Subcategories', i18n::$textdomain);
	}
	...
	private function has_subcategories(){
		if (is_product_category()) {
			...
			$childs = get_term_children(get_queried_object_id(), 'product_cat');
			if ( empty($childs) ){
				return false;
			}else{
				return true;
