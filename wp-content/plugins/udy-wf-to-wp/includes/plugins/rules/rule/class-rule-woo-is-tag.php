...
class Rule_Woo_Is_Tag extends Rule {
	...
	public function __construct( $condition = '', $values = array() ) {
		...
		$this->key   = 'woo_is_tag';
		$this->label = __( '[WOO] is tag', i18n::$textdomain );
	}
	...
	static function find_matching_items( $query ) {
		$data       = array();
		$tags = $tags = get_terms( 'product_tag' );
		foreach ( $tags as $tag ) {
	...
	public function get_ui( Form $form, $class ) {
		...
		foreach ($this->get_values() as $value){
			$tag = get_term($value, 'product_tag');
			if($tag){
