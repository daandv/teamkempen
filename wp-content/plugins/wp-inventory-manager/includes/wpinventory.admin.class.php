...
final class WPIMAdmin extends WPIMCore {
	...
	private static $admin_filters = array(
	...
	public static function instructions() {
		$total_items = self::$item->get_all( array(), TRUE );
		...
		usort( $messages, array( __CLASS__, 'usort_messages' ) );
	...
	public static function analysis_messages( $total_items = NULL ) {
		...
		if ( empty( $settings['license_key'] ) ) {
			$messages['license_key'] = array(
				'message' => sprintf( self::__( 'You have not entered a WP Inventory Manager license key in %sLicense Keys Settings%s.  You will not receive automatic updates to WP Inventory Manager.' ), $urls['settings_license'], $close_a ),
				'class'   => 'warning'
			);
		}
		...
		if ( empty( $settings['display_listing'] ) ) {
			$messages['display_listing'] = array(
		...
		if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_on_front' ) && in_array( get_option( 'page_on_front' ), $shortcodes ) && ! $settings['shortcode_on_home'] ) {
			$messages['shortcode_on_home'] = array(
		...
		if ( ! empty( $inactive_statii ) ) {
			...
			if ( $count_inactive_items && $count_inactive_items > 2 && ( $count_inactive_items / $all_items ) > .2 ) {
				...
				$messages['display_inactive'] = array(
					'message'  => $message,
					'class'    => $this_class,
					'priority' => $priority
				);
		...
		if ( $on_status_screen ) {
			...
			if ( ! empty( $mixed ) ) {
				foreach ( $mixed AS $field => $count ) {
					...
					$messages[ 'numeric_sort_' . $field ] = array(
		...
		if ( $reserve_on ) {
			...
			if ( empty( $settings['display_detail'] ) ) {
				...
				$messages['reserve_display_detail'] = array(
	 ...
	 */
	public static function dashboard_panel( $title, $primary_label, $count, $counts = array(), $class = 'success' ) {
	...
	public static function list_items() {
        ...
        <table class="grid itemgrid">
			...
			while ( $loop->have_items() ) {
				...
				if ( ( (int) self::$config->get( 'low_quantity_alert' ) ) && ( $wpinventory_item->inventory_quantity <= self::$config->get( 'low_quantity_amount' ) ) ) {
					$class = ' class="warning"';
				}
                ...
                <tr data-id="<?php echo $wpinventory_item->inventory_id; ?>"<?php echo $class; ?>>
                    ...
                    <td class="action">
						<?php if ( $edit_url ) { ?>
                            <a href="<?php echo $edit_url; ?>"><span class="dashicons dashicons-edit"></span><span
                                        class="tip"><?php self::_e( 'edit item' ); ?></span></a>
						<?php }
						if ( $delete_url ) { ?>
                            <a class="delete" data-name="<?php echo $wpinventory_item->inventory_name; ?>"
                               href="<?php echo $delete_url; ?>"><span class="dashicons dashicons-trash"></span><span
                                        class="tip"><?php self::_e( 'delete item' ); ?></span></a>
						<?php }
						if ( $duplicate_url ) { ?>
                            <a class="duplicate" data-name="<?php echo $wpinventory_item->inventory_name; ?>"
                               href="<?php echo $duplicate_url; ?>"><span class="dashicons dashicons-admin-page"></span><span
            ...
            <div class="description wpim-last-updated"><?php
				if ( ! empty( $inventory_date_updated ) ) {
					...
					if ( ! empty( $inventory_updated_by ) ) {
						...
						$link         = '<a href="' . $updated_url . '">' . $updated_name . '</a>';
	...
	public static function save_category() {
		...
		$category_description = self::request( 'category_description' );
	...
	public static function edit_display() {
		...
		foreach ( (array) $display_fields AS $screen => $fields ) {
			...
			foreach ( $fields AS $sort => $key ) {
				...
				if ( $data ) {
					$selected_fields[ $screen ] .= '<li data-field-id="' . $key . '">' . $data['label'] . '</li>';
				}
			}
		}
		...
		?>
        <style>
            .inventorywrap div.list {
                width: <?php echo ((100 /  $count) - 2); ?>%;
            }

            .inventorywrap div.list + div.list:nth-child(<?php echo $count + 1; ?>n) {
                margin-left: 0;
            }
        </style>
        <form method="post" action="<?php echo self::$self_url; ?>">
            ...
            <div class="lists">
				...
				<?php }
				do_action( 'wpim_admin_display_lists' ); ?>
            </div>
            <table>
                <tr>
          ...
          jQuery( function ( $ ) {
            ...
            $( '.sortable' ).sortable( {
              ...
              receive: function ( event, ui ) {
                ...
                var ignore   = wpimIgnore[ receiver.replace( 'selected_', '' ) ];
	...
	public static function edit_settings() {
		...
		$reg_tip['core'] = ( empty( $reg_info['expires'] ) || ( $reg_info['expires'] < time() ) ) ? '<p class="license license_invalid">' . self::__( 'License key is invalid or expired.' ) . '</p><p class="description">A valid license key entitles you to support.  In addition, all paid add ons require a valid license of the WP Inventory Manager.</p>' : '<p class="license license_valid">' . sprintf( self::__( 'License key is valid until %s' ), self::format_date( $reg_info['expires'] ) ) . '</p>';

		foreach ( $add_ons AS $key => $add_on ) {
			$reg_info = ( isset( $all_reg_info[ $add_on->key ] ) ) ? $all_reg_info[ $add_on->key ] : FALSE;
			$class    = 'invalid';
			if ( empty( $reg_info['expires'] ) ) {
				$tip = self::__( 'License key is invalid' );
			} else if ( $reg_info['expires'] < time() ) {
				$tip = sprintf( self::__( 'License key expired on %s' ), self::format_date( $reg_info['expires'] ) );
			} else {
				$tip   = sprintf( self::__( 'License key is valid until %s' ), self::format_date( $reg_info['expires'] ) );
				$class = 'valid';
			}

			$reg_tip[ $add_on->key ] = '<p class="license license_' . $class . '">' . $tip . '</p>';
		}
        ...
        <form method="post" action="<?php echo self::$self_url; ?>" class="inventory-config">
            ...
            <table class="form-table">
                <tr class="subtab">
                    <th colspan="2"><h4><?php self::_e( 'License Keys' ); ?></h4></th>
                </tr>
                <tr>
				...
				foreach ( $add_ons AS $add_on ) {
					...
					echo '<td><input type="text" class="large-text" style="max-width:300px;" name="license_key_' . $add_on->key . '" value="' . esc_attr( $value ) . '" > ';
					echo $reg_tip[ $add_on->key ];
					echo '</td></tr>';
				}

				echo '<tr><th></th><td><p class="submit"><input type="submit" class="button button-small" name="save-force" value="' . self::__( 'Validate Licenses' ) . '" /></p></td></tr>';
				?>
            </table>
            ...
            <table class="form-table">
                ...
                <tr class="seo_urls">
                    ...
                    <td><input type="text" class="medium-text" name="seo_endpoint"
                               value="<?php echo esc_attr( $settings['seo_endpoint'] ); ?>"/></td>
                </tr>
                <tr class="seo_urls">
                ...
                <tr class="subtab">
                    <th colspan="2"><h4><?php self::_e( 'Dashboard Admin' ); ?></h4>
                    </th>
                </tr>
                <tr>
            ...
            <table class="form-table">
                ...
                <tr class="reserve">
                    ...
                    <td><textarea
                        ...
                        <p class="description"><?php self::_e( 'Message displayed when a visitor completes a reservation.' ); ?></p></td>
                </tr>
                <tr class="subtab">
            ...
            <h3><?php self::_e( 'Notifications' ); ?></h3>
            <table class="form-table">
                <tr class="subtab">
                    <th colspan="2"><h4><?php self::_e( 'Notifications' ); ?></h4></th>
                </tr>
                <tr>
                    <th><?php self::_e( 'Send Low Quantity Email Notification' ); ?></th>
                    <td>
						<?php echo self::dropdown_yesno( "low_quantity_email_check", $settings['low_quantity_email_check'] ); ?>
                    </td>
                </tr>
                <tr>
                    <th><?php self::_e( 'Show alert marker and message' ); ?></th>
                    <td><?php echo self::dropdown_yesno( "low_quantity_alert", $settings['low_quantity_alert'] ); ?><p class="description"><?php echo self::_e( 'Highlights the inventory item in the admin dashboard with a red-ish background. Also adds a message to the top of the edit item page.' ); ?></p></td>
                </tr>
                <tr class="email_notifications">
                    <th><?php self::_e( 'Email' ) ?></th>
                ...
                <tr class="email_notifications">
                    <th><?php self::_e( 'Quantity' ) ?></th>
                    <td><input type="number" name="low_quantity_amount"
                               value="<?php echo esc_attr( $settings['low_quantity_amount'] ); ?>">
                        <p class="description"><?php self::_e( 'This is the quantity the alert will be sent at. Once an item hits this quantity you will receive an email if you selected to receive them above.' ); ?></p>
                    </td>
                </tr>
	...
	private static function save_settings() {
		...
		/**
		 * At this point, it's too late to render the message AND get a redirect.
		 * Strategy is to "Buffer" the message(s) in a transient, and then to
		 * pick those up and display them later.
		 */
		ob_start();
		self::update_reg_key( $_POST );
		$messages = ob_get_clean();

		self::update_transient_messages( $messages );
	...
	public static function update_reg_key( $settings = [] ) {
		$scheduled_update = FALSE;

		// Function will accept empty settings for scheduled checks, this loads from saved settings
		if ( empty( $settings ) ) {
			$scheduled_update = TRUE;
			$settings         = self::$config->get_all();
		}

		// Triggered by the "Update Licenses" button in the Settings page
		if ( ! empty( $_POST['save-force'] ) ) {
			$scheduled_update = TRUE;
		}
	...
	private static function compare_sort_field( $sort_field ) {
		...
		if ( FALSE !== stripos( $current, 'CAST(' ) ) {
			$current = trim( str_ireplace( array( 'CAST(', ' AS DECIMAL)', '`' ), '', $current ) );
		}

		if ( FALSE !== stripos( $current, ' ASC' ) || FALSE !== stripos( $current, ' DESC' ) ) {
			$current = trim( str_replace( array( ' ASC', ' DESC' ), '', $current ) );
		}
