...
Class WPIMBXSlider extends WPIMCore {
	...
	private static $setting_types = array(
		'bxslideshow_mode'                 => array( 'mode', 'string' ),
		'bxslideshow_speed'                => array( 'speed', 'int' ),
		'bxslideshow_duration'             => array( 'duration', 'int' ),
		'bxslideshow_auto'                 => array( 'auto', 'bool' ),
		'bxslideshow_autohover'            => array( 'autoHover', 'bool' ),
		'bxslideshow_autocontrols'         => array( 'autoControls', 'bool' ),
		'bxslideshow_captions'             => array( 'captions', 'bool' ),
		'bxslideshow_custompager'          => FALSE,
		'bxslideshow_adaptiveheight'       => array( 'adaptiveHeight', 'bool' ),
		'bxslideshow_adaptiveheight_speed' => array( 'adaptiveHeightSpeed', 'init' ),
		'bxslideshow_width'                => FALSE,
		'bxslideshow_random'               => array( 'randomStart', 'bool' ),
		'bxslideshow_infinite'             => array( 'infiniteLoop', 'bool' ),
		'bxslideshow_hidecontrols'         => array( 'hideControlOnEnd', 'bool' ),
	);

	private $slideshow_settings = array();
	...
	public function __construct() {
		...
		add_action( 'wpim_edit_settings_media', array( $this, 'wpim_edit_settings_media' ) );
		add_action( 'wpim_footer', array( $this, 'wpim_footer' ) );
	...
	public function wpim_edit_settings_media() {
		...
		echo '<th colspan="2"><h4>' . $this->__( 'Slideshow Settings' ) . '</h4></th>';
		echo '</tr>';
		echo '<tr>';
		echo '<th for="bxslideshow_mode">' . $this->__( 'Slide Mode' ) . '</th>';
		echo '<td>';
		echo WPIMCore::dropdown_array( 'bxslideshow_mode', $this->slideshow_settings['bxslideshow_mode'], $mode_options );
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>' . $this->__( 'Speed' ) . '</th>';
		echo '<td>';
		...
		echo '<p class="decription">Defaults to 500 if left blank (milliseconds).</p>';
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>Auto</th>';
		...
		echo '</tr>';
		echo '<tr>';
		echo '<th>Auto Hover</th>';
		echo '<td>';
		echo WPIMCore::dropdown_yesno( 'bxslideshow_autohover', $this->slideshow_settings['bxslideshow_autohover'] );
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>Auto Controls</th>';
		echo '<td>';
		echo WPIMCore::dropdown_yesno( 'bxslideshow_autocontrols', $this->slideshow_settings['bxslideshow_autocontrols'] );
		echo '<p class="description">"Auto" above must be set to yes for this setting to work</p>';
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>Use Captions</th>';
		echo '<td>';
		echo WPIMCore::dropdown_yesno( 'bxslideshow_captions', $this->slideshow_settings['bxslideshow_captions'] );
		echo '<p class="description">This uses the "title" attribute of the image.  You can edit this in your media or set it when uploading a new image.</p>';
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>Use Thumbnails as pager</th>';
		echo '<td>';
		...
		echo '<p class="description">Default is 100% - enter any dimension you like (for example, 500px)</p>';
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>Random start</th>';
		echo '<td>';
		echo WPIMCore::dropdown_yesno( 'bxslideshow_random', $this->slideshow_settings['bxslideshow_random'] );
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>Infinite loop</th>';
		echo '<td>';
		echo WPIMCore::dropdown_yesno( 'bxslideshow_infinite', $this->slideshow_settings['bxslideshow_infinite'] );
		echo '<p class="description">Set this option for the show to go back to the first slide at the end and vice versa.</p>';
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>Hide controls on end</th>';
		echo '<td>';
		echo WPIMCore::dropdown_yesno( 'bxslideshow_hidecontrols', $this->slideshow_settings['bxslideshow_hidecontrols'] );
		echo '<p class="description">Set this option to take away the next button at the end of the show and vice versa. This requires that infinite loop above <strong>not be set</strong>.</p>';
		echo '</td>';
		echo '</tr>';
		echo '<tr>';
		echo '<th>Use adaptive height</th>';
		echo '<td>';
		...
		echo '</tr>';
		echo '<tr>';
		echo '<th>Adaptive height speed</th>';
		echo '<td>';
		...
		echo '<p class="description">' . $this->__( 'This is measured in milliseconds.  It require that "Adaptive Height" above be set to "Yes".  If left blank it defaults to 500.' ) . '</p>';
		echo '</td>';
		echo '</tr>';
	}
