...
//				$new_order[ $i ] = "udf_NaturalSortFormat({$field}, 10, '.'){$this_sdir}";
	...
	private function check_tables() {
		...
		if ( version_compare( $inventory_version, '1.2.6' ) < 0 ) {
			...
			$keep_theme = array( 'Default Theme' );
		...
		if ( apply_filters( 'wpim_do_fresh_install', self::$fresh_install ) ) {
			...
			add_action( 'admin_init', array( $default, 'install' ) );
		}
	 ...
	 * @return string
	 */
	public function parse_custom_where( $where, $args = array() ) {
		if ( ! empty( $args ) ) {
			$wheres = array();
			$start  = FALSE;
		...
		$where = str_ireplace( array( '&#8221;', '&#8243' ), '', $where );

		$where = html_entity_decode( $where );

		$where = str_ireplace( array( ';' ), '', $where );
	 ...
	 * Handles multiple values (comma-separated), turning into an IN clause.
	 *
	 * @param             $value
