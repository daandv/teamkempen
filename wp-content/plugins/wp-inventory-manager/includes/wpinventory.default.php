...
Class WPIMDefaultItems extends WPIMCore {
	...
	public function delete_default_data() {
		...
		foreach ( $data['images'] AS $image_id ) {
			...
			$this->wpdb->delete( $this->db->image_table, array( 'image_id' => $image_id ) );
		...
		foreach ( $data['media'] AS $media_id ) {
			...
			$this->wpdb->delete( $this->db->media_table, array( 'media_id' => $media_id ) );
		...
		foreach ( $data['categories'] AS $category_id ) {
			$this->wpdb->delete( $this->db->category_table, array( 'category_id' => $category_id ) );
		}
