...
class WPIMPromo extends WPIMCore {
	...
	public function render_checkout_link( $id ) {
		...
		return '<a class="button button-primary green" href="https://www.wpinventory.com/checkout?edd_action=add_to_cart&download_id=' . $id . '&email=' . get_option('admin_email') . '" target="_blank">' . self::__( 'Buy Now' ) . '</a>';
	}
	...
	private function promote_all_access_pass() {
		...
		echo '<div class="promotion_ctas"><a href="https://www.wpinventory.com/downloads/wp-inventory-manager-all-access-pass/" target="_blank">Learn More</a> <a class="button button-primary green" href="https://www.wpinventory.com/checkout?edd_action=add_to_cart&download_id=1990&email=' . get_option('admin_email') .'" target="_blank">' . self::__( 'Buy Now' ) . '</a></div>';
		echo '</div>';
	...
	public function promote_ie() {
		...
		// TODO: Cale, please make this sentence appear in all promos please and only show it if the install DOES NOT have an active WPIM license
