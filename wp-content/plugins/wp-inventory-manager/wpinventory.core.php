...
class WPInventoryInit extends WPIMCore {
	...
	private static function add_actions() {
		...
		add_filter( 'wpim_shortcode_rendered', array( __CLASS__, 'wpim_shortcode_rendered' ) );
		add_filter( 'wpim_display_filter_form', array( __CLASS__, 'wpim_display_filter_form' ) );
		add_filter( 'wpim_display_pagination', array( __CLASS__, 'wpim_display_pagination' ) );
		add_filter( 'wpim_loop_query_args', array( __CLASS__, 'wpim_loop_query_args' ) );
	...
	private static function add_submenu( $title, $slug, $role = 'manage_options', $class = '', $method = '' ) {
		...
		add_submenu_page( self::MENU, $title, $title, $role, $slug, array(
			$class,
			$method
		) );
	...
	public static function load_font_awesome() {
		...
		if ( ! in_array( 'font-awesome.css', $styles ) && ! in_array( 'font-awesome.min.css', $styles ) ) {
			wp_enqueue_style( 'font-awesome', self::$url . 'vendor/font-awesome-4.7.0/css/font-awesome.min.css', array(), '4.7.0' );
		}
	}
	...
	public static function wp_enqueue_scripts() {
		...
		wp_register_script( 'wpinventory-common', self::$url . 'js/wpinventory.js', array( 'jquery' ), self::VERSION, TRUE );
	...
	public static function register_common_scripts() {
		wp_register_script( 'wpinventory-toastr', self::$url . 'vendor/toastr/toastr.min.js', array( 'jquery' ), self::VERSION, TRUE );
		wp_register_script( 'wpinventory-datatables', self::$url . 'vendor/DataTables/datatables.min.js', array( 'jquery' ), self::VERSION, TRUE );
	...
	/**
	 * Checks all licenses to ensure current, valid.
	 */
	private static function plugin_updater() {
		$add_ons = self::get_full_add_ons();

		if ( ! $add_ons ) {
			$add_ons = self::get_full_add_ons( TRUE );
		}
		...
		// Loops through all of the add-ons (and core) and checks if the plugins have updates available
